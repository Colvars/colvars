...
proc ::cv_dashboard::load_cv_traj { filenames } {
...
proc ::cv_dashboard::get_local_dir {} {
   ...
   switch $::tcl_platform(platform) {
       "windows" {
           ...
           return [file join $base "cv_dashboard"]
       }
       "unix" {
           set home $::env(HOME)
           if {$::tcl_platform(os) eq "Darwin"} {
               return [file join $home "Library" "Application Support" "cv_dashboard"]
           } else {
               return [file join $home ".local" "share" "cv_dashboard"]
           }
       }
   }
}
...
proc ::cv_dashboard::self_update {{force false}} {
    ...
    # Check for updates
    if {!$force} {
        lassign [check_version_file] status latest_version
        if {$status eq "current"} {
            puts "Package is up to date (version $version)"
            return
            # return false
        }
        puts "Update available: $version -> $latest_version"
    }
...
proc ::cv_dashboard::cleanup_backups {{keep 3}} {
