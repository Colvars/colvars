...
proc ::cv_dashboard::self_update {{force false}} {
  ...
  if {!$force} {
    ...
    if {$status eq "current"} {
      puts "Local package ($version) is up to date"
      return false
    } elseif {$status eq "newer"} {
      puts "Local package ($version) is newer than remote ($latest_version)"
      return false
    }
...
# Try to update and switch to newer version when package is loaded
::cv_dashboard::self_update
::cv_dashboard::load_newer_version
