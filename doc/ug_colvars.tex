\ifdefined\NAMDCONF
\newcommand{\key}[5]{\NAMDCONF{#1}{(#2) #3}{#4}{#5}}
\newcommand{\keydef}[6]{\NAMDCONFWDEF{#1}{(#2) #3}{#4}{#5}{#6}}
\fi
...
CNRS, Paris, France). Please send feedback and suggestions
...
\fi
The module is designed to perform concurrently multiple tasks, the most common of which are:
\begin{itemize}
...
\item analyze an existing MD trajectory in terms of colvars: use NAMD's \texttt{coorfile read} command, and perform a 0-timestep run for each set of coordinates, as illustrated in \ref{section:sample}. 
\else%
.
\fi
...
In \ref{sec:colvar_namd_parameters}, we document the syntax of such parameters.
...
In \ref{sec:colvar_lammps_parameters}, we document the syntax of such parameters.
\fi
\fi
...
\fi
\label{sec:colvar_namd_parameters}

Two parameters must be added to the NAMD configuration file, \texttt{colvars} and \texttt{colvarsConfig}.
...
\else\ifdefined\LAMMPS
\subsection{LAMMPS parameters}
\label{sec:colvar_lammps_parameters}
\fi
\fi
...
NAMD configuration file.
The syntax of the colvars configuration file is similar to that of the NAMD file (see \ref{section:configsyntax}), with a few important notes:
\else
...
\item unlike in the NAMD main configuration file,  Tcl commands and variables are not available, but it is possible to use Tcl to generate a new configuration file with different parameters (see \ref{sec:colvar_namd_parameters});
\else
...
\item %
  \key{%
    indexFile}{%
    global context}{%
    Index file for atom selection (GROMACS format)}{%
    UNIX filename}{%
    This option allows to read an index file (usually named with a \texttt{.ndx}
    extension) formatted with the GROMACS syntax.
    The names of index groups contained in this file can then be used to define colvars.
...
\ifdefined\NAMDCONF
\subsubsection{Input files (optional)}
\else
\subsection{Input files (optional)}
\fi
...
\else\ifdefined\LAMMPS
The name of this file is provided as the argument to the \texttt{input} keyword of the \texttt{fix colvars}.
\fi
\fi
...
\item a \emph{state file}, named \texttt{$<$outputName$>$.colvars.state}; this file is in ASCII format, regardless of the value of \texttt{binaryOutput} in the NAMD configuration; to continue the simulation, the name of this file must be included in the configuration of the next run using \texttt{colvarsInput}, together with the other NAMD output files;
...
Selection is incrementali: each keyword adds the corresponding atoms to the selection, however, atoms included by multiple keywords are only counted once.
...
\end{verbatim}
The resulting selection includes atoms 1 to 3, those indicated by a flag in the given PDB file, as well as the C$_\alpha$ atoms selected by name, segment, and residue numbers. 
\\
...
\item %
  \key{%
    ...
    If non-PSF topologies are used in NAMD (e.g.~AMBER or GROMACS) this option is not necessary.}
...
Finally, a few components are defined by convention using a roto-translated frame (e.g. the minimal RMSD): for these components, NAMD enables \texttt{centerReference} and \texttt{rotateReference} by default.
...
\item %
  \keydef{%
    ...
    boolean}{%
    \texttt{off}}{%
    If this option is \texttt{on}, the center of geometry of the group will be aligned with that of the reference positions provided by either \texttt{refPositions} or \texttt{refPositionsFile}.
...
\item %
  \keydef{%
    ...
    boolean}{%
    \texttt{off}}{%
    If this option is \texttt{on}, the coordinates of this group will be optimally superimposed to the reference positions provided by either \texttt{refPositions} or \texttt{refPositionsFile}.
    ...
    The algorithm used is the same employed in the \texttt{orientation} colvar component     \cite{Coutsias2004}.
...
\item %
  \keydef{%
    ...
    The following example illustrate the syntax of \texttt{refPositionsGroup}: group called ``\texttt{atoms}'' is defined, including 8 C$_{\alpha}$ atoms of a protein of 100 residues.  An optimal roto-translation is calculated automatically by fitting the C$_{\alpha}$ trace of the rest of the protein onto the coordinates provided by a PDB file.}
\begin{verbatim}
...
The following two options have default values appropriate for the vast majority of applications, and are only provided for rare, special cases.
\begin{itemize}
...
\ifdefined\NAMDCONF
\paragraph*{Recommendations for using atom groups.}
\else
\subsection{Recommendations for using atom groups.}
\fi
When defining atom groups for a collective variable, these guidelines should be followed to avoid inconsistencies and performance losses:
...
\item In simulations with periodic boundary conditions, NAMD maintains
  ...
  \item[\emph{iii)}] it is used by a colvar component which does not
    access its center of geometry, and uses instead only pairwise
    distances (\texttt{distanceInv}, \texttt{coordNum}, \texttt{hBond}, \texttt{alpha}, \texttt{dihedralPC});
...
\item \textbf{Performance issues:}
  NAMD spreads the calculation of most interaction terms over many computational nodes, but the calculation of colvars is not parallelized.  Therefore, additional calculations are executed by the master node, and most importantly, additional communication is added with the other nodes.
...
\item %
  \key{%
    main}{%
    \texttt{distanceZ, \texttt{distanceXY}} context}{%
    Main group of atoms}{%
    Block \texttt{main \{...\}}}{%
...
\item %
  \keydef{%
    forceNoPBC}{%
    \texttt{distanceZ, distanceXY} context}{%
    Calculate absolute rather than minimum-image distance?}{%
    boolean}{%
    \texttt{no}}{%
...
\item \keydef{%
    oneSiteSystemForce}{%
    \texttt{angle}, \texttt{dihedral} context}{%
    Measure system force on group 1 only?}{%
    boolean}{%
    \texttt{no}}{%
    If this is set to \texttt{yes}, the system force is measured along
...
\ifdefined\NAMDCONF
\paragraph*{\texttt{rmsd}: root mean square displacement  (RMSD) with respect to a reference structure.}
\else
\subsubsection{\texttt{rmsd}: root mean square displacement  (RMSD) with respect to a reference structure.}
\fi
...
related options (\texttt{refPositionsFile}, etc.) explicitly in the
...
positional restraints within the Collective Variables Module;
...
\item %
  \keydef{%
    ...
    boolean}{%
    \texttt{off}}{%
    If this option is \texttt{on}, the numbers provided by \texttt{vector} or \texttt{vectorFile} are interpreted as another set of positions, $\mathbf{x}_{i}'$: the vector $\mathbf{v}_{i}$ is then defined as $\mathbf{v}_{i} = \left(\mathbf{x}_{i}' - \mathbf{x}_{i}^{\mathrm{(ref)}}\right)$.
...
For convenience, the vector is also normalized so that $\xi = 0$ when the atoms have positions A and $\xi = 1$ at the set of positions B.
    }
...
As for the component \texttt{rmsd}, the options \texttt{atoms}, \texttt{refPositions}, \texttt{refPositionsFile} and \texttt{refPositionsCol}, are supported.
Note in particular that \texttt{refPositions} and \texttt{refPositionsFile} define the set of positions from which the optimal rotation is calculated.
...
This can be defined using the same options as the component \texttt{orientation}: \texttt{atoms}, \texttt{refPositions}, \texttt{refPositionsFile}, and \texttt{refPositionsCol}).
In addition, \texttt{spinAngle} accepts the \texttt{axis} option:
\begin{itemize}
\item %
  \keydef{%
    axis}{%
    \texttt{tilt, spinAngle} context}{%
...
\item %
  \keydef{%
    wrapAround}{%
    \texttt{distanceZ}, \texttt{dihedral} or \texttt{spinAngle} context}{%
    Center
...
\begin{itemize}
\item \keydef{fullSamples}{\texttt{abf} context}{%
...
\item \keydef{hideJacobian}{\texttt{abf} context}{%
...
\item \keydef{outputFreq}{\texttt{abf} context}{%
...
\item \keydef{historyFreq}{\texttt{abf} context}{%
...
\item \key{inputPrefix}{\texttt{abf} context}{%
...
\item \keydef{applyBias}{\texttt{abf} context}{%
    Apply the ABF bias?}
  {boolean}
  {\texttt{yes}}
...
\item \keydef{updateBias}{\texttt{abf} context}{%
    Update the ABF bias?}
  {boolean}
  {\texttt{yes}}
...
\subsubsection{Performance tuning}
\fi
\label{sec:colvarbias_performance}
...
\item %
  \keydef{%
    ...
    temporarily written to the file ``\outputName\texttt{.colvars.%
...
\begin{itemize}
\item \keydef{outputFreq}{\texttt{histogram} context}{%
