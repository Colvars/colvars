...
This instructs the Dashboard to update the list of atoms whenever the configuration is applied, that is, when it is edited, when the file is loaded by the Dashboard, or when changing the molecule linked to Colvars.
This is useful when working across systems with different atom numberings, but topologies that make atom groups identifiable using VMD selection texts.
Special fields (O, B, and user) may be used, as well as atom positions (e.g. \texttt{z > 0}).
Note that the atom selections used by Colvars are not updated when changing frame within the same molecule (animating a trajectory).
...
\begin{itemize}
\cvvmdonly{
\item \texttt{molid} [\emph{$<$molid$>$}]: setup the Colvars module for the given molecule; \emph{$<$molid$>$} is the identifier of a valid VMD molecule, either an integer or \texttt{top} for the top molecule; without argument, return the molid currently associated with Colvars.
...
\cvsubsubsec{\texttt{deerkernel}: Double Electron-Electron Resonance (DEER) kernel function.}{sec:cvc_deerkernel}
\labelkey{colvar|deerkernel}

The \texttt{deerkernel \{...\}} block defines a DEER kernel function component that depends on the distance between the centers-of-mass of two atom groups, \texttt{group1} and \texttt{group2}.
The DEER kernel function reflects the intra-molecular component of the DEER time-signal associated to the dipolar interaction between the electron spin-labels attached to a biomolecule. In this case we refer to a dilute solution of a biomolecule with two attached spin-labels, for which the DEER kernel can be experessed as a function of the distance between the centers-of-mass of the spin-labels radical groups (e.g. the nitroxide groups of MTSSL spin-labels) \cite{Milov1997}:

\begin{equation}
\label{eq:deerkernel}
k(t_{d},r) = \sqrt{\pi[C(z)^{2}+S(z)^{2}]}
\end{equation}


\begin{cvcoptions}
\item %
  \labelkey{colvar|deerkernel|group1}
  \key
    {group1}{%
    \texttt{deerkernel}}{%
    First group of atoms}{%
...
\item %
  \labelkey{colvar|deerkernel|group2}
  \simkey{group2}{\texttt{deerkernel}}{group1}

\item %
  \labelkey{colvar|deerkernel|forceNoPBC}
  \keydef
    {forceNoPBC}{%
    \texttt{deerkernel}}{%
    Calculate absolute rather than minimum-image distance?}{%
    ...
    \texttt{no}}{%
    By default, in calculations with periodic boundary conditions, the
    \texttt{deerkernel} component returns the distance according to the
    minimum-image convention. If this parameter is set to \texttt{yes},
    PBC will be ignored and the distance between the coordinates as maintained
    internally will be used. This is only useful in a limited number of
    special cases, e.g. to describe the distance between remote points
...
\item %
  \labelkey{colvar|deerkernel|oneSiteTotalForce}
  \keydef
    {oneSiteTotalForce}{%
    \texttt{angle}, \texttt{dipoleAngle}, \texttt{dihedral}}{%
    Measure total force on group 1 only?}{%
...
\item %
  ...
  \keydef
    ...
    This number can be, for example, a true physical boundary imposed by the choice of function (e.g.{} the \texttt{distance} function is always larger than zero): if this is the case, and only one function is used to define the variable, tthe default value of this number is set to the lowest end of the range of values of that function, if available (see Section~\ref{sec:cvc_list}).
...
\item %
  ...
  \keydef
    ...
    For more complex variable definitions, or when \refkey{lowerBoundary}{colvar|lowerBoundary} is provided directly by the user, it may be necessary useful to set this flag explicitly.
