...
The variables discussed in this section quantify the rotations of macromolecules (or other quasi-rigid objects) from a given set reference coordinates to the current coordinates.
Such rotations are computed following the same method used for best-fit RMSDs (see \refkey{rmsd}{colvar|rmsd} and \refkey{fittinGroup}{atom-group|fittinGroup}).
...
Although $\mathsf{q}$ is used internally in the Colvars module for features such as the \refkey{rmsd}{colvar|rmsd} function and the \refkey{fittinGroup}{atom-group|fittinGroup} option, its direct use as a collective variable is more difficult, and mostly limited to fixed or moving restraints.
...
% TODO Adjust when the range of values of eulerTheta is defined
A slightly more complete parametrization decomposition into the two parameters, \refkey{tilt}{colvar|tilt} and \refkey{spinAngle}{colvar|spinAngle}.
...
This parameterization is mathematically well defined for almost all rotations, including small ones when the current coordinates are almost completely aligned with the reference ones.
However, the presence of a mathematical singularity at $\theta = 90^\circ$ prevents using the \refkey{spinAngle}{colvar|spinAngle} function near those configurations.
For these reasons, \refkey{tilt}{colvar|tilt} and \refkey{spinAngle}{colvar|spinAngle} are useful when the possible rotations are known to have approximately the same axis.
In this use case, \refkey{spinAngle}{colvar|spinAngle} measures the angle of rotation around the chosen axis $\mathbf{e}$, whereas \refkey{tilt}{colvar|tilt} measures the dot product between $\matbhf{e}$ and the actual axis of the full rotation.
...
can be defined as the intersection of the plane \textit{xy} and \textit{XY}. In 
this case, as illustrated in the figure below, the complete 
...
\item[\emph{(ii)}] the ``pitch'' angle $\theta$, i.e.{} the rotation angle from the \textit{z} axis to the \textit{Z} axis, and
...
\fi
\end{center}
\noindent{}\rightline{\emph{(adapted from the \href{https://en.wikipedia.org/wiki/Euler\_angles}{Euler angles Wikipedia page} under the \href{https://creativecommons.org/licenses/by/3.0}{CC BY 3.0} license)}}
...
measures the rotation angle from the \textit{z} axis to the \textit{Z} axis. 
