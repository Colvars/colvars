...
Detailed explanations of the design of the Colvars module are provided in ref.\ \citealp{Fiorin2013}.  Please cite this reference whenever publishing work that makes use of this module, alongside any other publications for specific features being, according to the usage summary printed when running a Colvars-enabled MD simulation or analysis.
...
Here, we document the syntax of the commands and parameters used to set up and use the Colvars module in \MDENGINE{}\cvgromacsonly{ \cite{Abraham2015}}\cvlammpsonly{ \cite{Thompson2022}}\cvnamdonly{ \cite{Phillips2020}}.
...
\item %
  \key
    ...
    If file name contains ``\texttt{.colvars.state}'', Colvars will strip any further suffix following this string.
    If not, the suffix wil be appended.
    When intending to load an input state file after startup, e.g.\ within a Tcl script section, the \texttt{cv load} command should be used instead.
  }
...
Anything following the ``\texttt{.colvars.state}'' suffix will be stripped.
...
\cvgromacsonly{\cvsubsubsec{Restarting in GROMACS.}{sec:colvars_input_gromacs}
  \ifdefined\gromacsmdmodule
...
\cvlammpsonly{\cvsubsubsec{Restarting in LAMMPS.}{sec:colvars_input_lammps}
...
\cvnamdonly{\cvsubsubsec{Restarting in NAMD.}{sec:colvars_input_namd}
When the initial configuration is provided using the NAMD keyword \texttt{colvarsConfig}, the \texttt{colvarsInput} NAMD keyword can be used to give the name of a state file, which will be read immediately at startup.
On the other hand, for loading an input state file after startup, the Tcl command \texttt{cv load} (see \ref{sec:cv_scripting} and \ref{sec:cv_command_loadsave}) is also available.
Note that the two approaches \emph{are not interchangeable}: because the \texttt{colvarsConfig} and \texttt{colvarsInput} keywords are processed at NAMD startup, their effect \emph{precedes} any command included in the Tcl-scripted portion of a NAMD simulation script, including \texttt{cv load}.

To ensure that the Colvars configuration is always loaded before an input state file is read, one should \emph{either}:
\begin{itemize}
\item use the keywords \texttt{colvarsConfig} and \texttt{colvarsInput} if the Colvars configuration never changes during the NAMD run, \emph{or}
\item use the Tcl commands \texttt{cv config} (or \texttt{cv configfile}) and \texttt{cv load} to manage Colvars from the Tcl-scripted portion of a NAMD input script.
\end{itemize}

The \texttt{colvarsInput} keyword and the \texttt{cv load} command both support loading Colvars state files in either binary or text formats.
} % end \cvnamdonly
...
For the sake of clarity, we recommend that as soon as a suitable GROMACS checkpoint becomes available, the \texttt{defaultInputStateFile} is \emph{removed} and a new TPR file is produced accordingly.
...
ref.\ \citealp{Coutsias2004}, which guarantees a continuous
dependence of
$U^{\{\mathbf{x}_{i}(t)\}\rightarrow\{\mathbf{x}_{i}^{\mathrm{(ref)}}\}}$
...
\item %
  ...
  \key
    ...
    is disabled altogether. For details, see ref.\ \citealp{Ebrahimi2022}.
  }
\end{cvcoptions}
...
The underlying mathematical formalism is described in refs.\ \citealp{Coutsias2004,Daniilidis1999,Hu2006}, and the implementation in ref.\ \citealp{Fiorin2013}.
...
Although $\mathsf{q}$ is used internally in the Colvars module for features such as the \refkey{rmsd}{colvar|rmsd} function and the \refkey{fittingGroup}{atom-group|fittingGroup} option, its direct use as a collective variable is mostly limited to fixed or moving restraints.
...
These quantify the amplitudes of two independent sub-rotations away from a certain axis $\mathbf{e}$, and around that axis $\mathbf{e}$, respectively.
The axis $\mathbf{e}$ is chosen by the user, and is by default the Z axis:
under that choice, \refkey{tilt}{colvar|tilt} is equivalent to the sine of the Euler ``pitch'' angle $\theta$, and \refkey{spinAngle}{colvar|spinAngle} to the combined effect of the other two angles $\phi$ and $\psi$.
This parameterization is mathematically well defined for most orientations, including when the current coordinates are almost completely aligned with the reference ones.
However, a mathematical singularity affects the \refkey{spinAngle}{colvar|spinAngle} function near configurations where the value of the corresponding \refkey{tilt}{colvar|tilt} is -1 (i.e.\ a $180^\circ$ rotation around an axis orthogonal to $\mathbf{e}$).
Therefore, \refkey{tilt}{colvar|tilt} and \refkey{spinAngle}{colvar|spinAngle} are most useful together when it is known that the range of possible orientations are somewhat limited.
For example, a membrane protein that is unable to fully flip around the membrane plane, or a molecule whose \refkey{tilt}{colvar|tilt} has a wall restraint applied to it; example inputs for each case are provided below.
...
ref.\ \citealp{Coutsias2004}.
The quaternion $(q_0, q_1, q_2, q_3)$
...
``\texttt{(0.707, 0.0, 0.0, 0.707)}''.
For any quaternion $q$, $q$ and $-q$ represent the same rotation. Therefore, by convention, \texttt{orientation} always returns a quaternion verifying $q_0 \ge 0$. This ensures that \texttt{orientation} is a unique and continuously-varying function of Cartesian coordinates.
The script
...
The full rotation from the current coordinates $\left\{\mathbf{x}_{i}\left(t\right)\right\}$ and the
reference coordinates $\left\{\mathbf{x}_{i}^{\mathrm{(ref)}}\right\}$, can be decomposed into two sub-rotations:
\begin{itemize}
\item a ``\emph{spin}'' rotation of angle $\eta$ around \textbf{e}
\item a ``\emph{tilt}'' rotation of angle $\omega$ around an axis orthogonal to \textbf{e}.
\end{itemize}
The schematic below, adapted from ref.\ \citealp{Fiorin2024}, illustrates the use of the two angles to model the rotations of a membrane protein:
\begin{center}
\ifdefined\HCode
\includegraphics{spin-tilt-scheme-512px.png}
\else
\includegraphics[width=\textwidth]{spin-tilt-scheme.pdf}
\fi
\end{center}
If the axis \texttt{e} is the Cartesian Z-axis, $\eta$ can be thought of as combining the effect of the two Euler angles $\phi$ and $\psi$, $\omega$ has a similar role to the Euler angle $\theta$, and the axis orthogonal to \texttt{e} is the line of nodes.
Compared to the Euler angles, the spin angle $\eta$ is well defined even if the current coordinates are near the reference coordinates ($\mathbf{x}_{i}\left(t\right) \approx \mathbf{x}_{i}^{\mathrm{(ref)}}$), or more broadly, $\omega \approx 0$: at that condition, the spin angle $\eta$ is preferable to the Euler angles to avoid a ``\href{https://en.wikipedia.org/wiki/Gimbal_lock}{gimbal lock}''.

Note that numerical issues may still arise in the diametrically opposite configuration where $\omega \approx 180^{\circ}$ (see the expression for $\nabla_{\mathbf{X}}\phi$ in Table~A5 in ref.\ \citealp{Fiorin2013} when ${q_{0}}^{2} \approx 0$ and $\left({\mathbf{q}}\cdot\mathbf{e}\right)^{2} \approx 0$).
Under that scenario, neither the spin angle nor the Euler angles are suitable collective variables.

\textbf{Tip:} To avoid such configurations altogether, consider defining a colvar of type \refkey{tilt}{colvar|tilt} using the same axis \textbf{e}, and restraining it to gave only positive values, i.e.\ $\cos(\omega) > 0$.

The colvar component \texttt{spinAngle} measures the angle of the ``spin'' sub-rotation around the chosen axis \textbf{e}.
For a quaternion $\mathsf{q} = (q_0, q_1, q_2, q_3) = (q_0, \mathbf{q})$ that represents the full
rotation, the spin angle $\eta$ of the
sub-rotation is defined by this expression \cite{Daniilidis1999,Hu2006}:
\begin{equation}
  \label{eq:cvc_spinAngle}
  \eta \left(\mathbf{e}, \mathsf{q}\right) = 2 \tan^{-1}\left(\dfrac{\mathbf{e} \cdot \mathbf{q}}{q_0}\right)
\end{equation}
...
Its value is a continuous function almost everywhere, with the exception of configurations with the corresponding ``tilt'' angle is near 180$^\circ$ (i.e.~the \texttt{tilt} component is near $-1$).

\paragraph*{Example: stopping the rotation of a membrane protein around the Z-axis.}  To prevent a membrane protein from ``spinning'' around the membrane normal (such rotations are usually irrelevant when the membrane is homogeneous and periodic), the restraint shown in the following example may be used.
This restraint allows its internal motions and changes in tilt angle, which are physically meaningful.
\cvnamdonly{In NAMD, the user should also take care that the protein is composed by a single chain, or disable \texttt{wrapAll} otherwise.}

\begin{cvexampleinput}
\-colvar~\{\\
\-~~name~p\_spin\\
\-~~spinAngle~\{\\
\-~~~~atoms~\{~(your~selection~here)~\}\\
\-~~~~refPositionsFile~reference.pdb\\
\-~~~~axis~(0.0,~0.0,~1.0)~~~\#~This~is~also~the~default\\
\-~~\}\\
\-\}\\
\-\\
\-harmonic~\{~~~~~~~~~~~~~~~~~\#~Define~a~harmonic~restraint\\
\-~~colvars~p\_spin~~~~~~~~~~~\#~acting~on~colvar~"spin"\\
\-~~centers~0.0\\
\-~~forceConstant~10.0~~~~~~~\#~unit:~energy/degrees**2\\
\-\}\\
\end{cvexampleinput}
...
The component \texttt{tilt} measures the cosine of the angle $\omega$ of the ``tilt'' sub-rotation away from a chosen axis $\mathbf{e}$ (see also \refkey{spinAngle}{colvar|spinAngle}).

For a quaternion $\mathsf{q} = (q_0, q_1, q_2, q_3) = (q_0, \mathbf{q})$ that represents the full rotation, a chosen axis $\mathbf{e}$ and the spin angle $\eta$ around that axis (keyword \refkey{spinAngle}{colvar|spinAngle}), the cosine of the tilt angle $\omega$ is defined as \cite{Daniilidis1999,Hu2006}:
\begin{equation}
  \label{eq:cvc_tilt}
  t\left(\mathbf{e}, \mathsf{q}\right) = \cos\left(\omega\left(\mathbf{e}, \mathsf{q}\right)\right) = 2 \left(\dfrac{q_0}{\cos\left(\eta/2\right)}\right)^2 - 1
\end{equation}
where the function is extended by continuity for at $q_0 = \cos\left(\eta/2\right) = 0$ (full rotation of $\pm 180^{\circ}$ around $\mathbf{e}$).
Notably, the cosine of the tilt angle $\cos\left(\omega\right)$ is here provided, rather than the angle itself, because the latter is unevenly distributed even for an isotropic system: consider as an analogy the angle $\theta$ in the spherical coordinate system.
...
The values of \texttt{tilt} are real numbers in the interval $[-1:1]$: the value $1$ represents an orientation fully parallel to \textbf{e} (tilt angle $\omega = 0^\circ$), and the value $-1$ represents an anti-parallel orientation (tilt angle $\omega = 180^\circ$).

\paragraph*{Example: keeping a macromole orientation within 60$^\circ$ from a chosen axis.}  To prevent a molecule to lose orientation with respect to an axis, while allowing its internal motions and rotations around that axis unperturbed, the following example may be used.
The lower wall of 0.5 for the variable ``\texttt{mol\_tilt}'' represents an upper wall on the tilt angle $\omega$ at 60$^\circ$.
\cvnamdonly{In NAMD, the user should also take care that the protein is composed by a single chain, or disable \texttt{wrapAll} otherwise.}
...
\-\}\\
\-\\
\-harmonicWalls~\{~~~~~~~~~~~~\#~Define~a~harmonic~wall\\
\-~~colvars~mol\_tilt~~~~~~~~~\#~acting~on~colvar~"t"\\
\-~~lowerWalls~0.5\\
\-~~forceConstant~100.0~~~~~~\#~unit:~energy~(tilt~is~dimensionless)\\
\-\}\\
\end{cvexampleinput}
...
Therefore, a safe use of Euler angles as collective variables requires the use of restraints to avoid such singularities, such as done in ref.\ \citealp{Fu2017} and in the \href{http://www.ks.uiuc.edu/Training/Tutorials/namd/PLB/tutorial-protein-ligand.pdf}{protein-ligand binding NAMD tutorial}.
...
\end{eqnarray}
where $C_{\text{hb}}$ is defined by \refkey{hBondCoeff}{colvar|alpha|hBondCoeff},
...
\begin{figure}[!ht]
	\centering
	\ifdefined\HCode{\HCode{<img class="neural_network" src="dense_nn.png" alt="Dense neural network">}}\else\includegraphics[width=9cm]{dense_nn}\fi
...
\cvsubsec{Torch-based collective variables}{sec:cvc_torchann}

The \texttt{torchANN} keyword defines a scalar component based on an artificial neural network (ANN) model previously trained using PyTorch. Similarly to the \texttt{linearCombination} and \texttt{neuralNetwork} components, the ANN model is expressed as a function of collective variable components. The model is loaded from a PyTorch-formatted file, defined by the keyword \texttt{modelFile}\cvgromacsonly{; the contents of this file are embedded in the TPR}.

This component requires that \MDENGINE{} is linked against the libTorch library; see \ref{sec:compilation_notes} for more details.

\begin{cvcoptions}

\item %
  \labelkey{colvar|torchANN|modelFile}
  \key
    {modelFile}{%
    \texttt{torchANN}}{%
    ...
    The model is assumed to take a one-dimensional tensor (excluding batch dimension) as input.
  }
...
\item %
  ...
  \keydef
    ...
    0-based integer}{%
    0}{%
    This option specifies the index of the output node to be used as the value of the collective variable (see also the equivalent keyword \refkey{output\_component}{colvar|NeuralNetwork|output_component} in the built-in neural network implementation).
  }

\item %
  \labelkey{colvar|torchANN|doubleInputTensor}
  \keydef
    {doubleInputTensor}{%
    \texttt{torchANN}}{%
    Specify that the input model is in double precision}{%
    boolean}{%
    \texttt{off}}{%
    This flags controls the precision of the PyTorch model contained in \texttt{modelFile}.
  }

\end{cvcoptions}

An example of creating a model file and defining a collective variable component using \texttt{torchANN} is shown below:

\begin{mdexampleinput}
\-import torch\\
\-import torch.nn as nn\\
\\
\-\#~The~neural~network~model is a function: \\
\-\#~~~~~$f=(f_1,f_2,f_3): \mathbb{R}^8\rightarrow \mathbb{R}^3$.\\
...
\begin{cvexampleinput}
\-colvar~\{\\
\-~~\#~Define a scalar CV as $\xi=f_1(x_2,y_2,z_2,x_4,y_4,z_4, \phi, \psi)$, \\
\-~~\#~i.e.~the function represented by the first component of the Torch model, \\
\-~~\#~evaluated at the coordinates of the two atoms with atomNumbers 2 and 4,\\
\-~~\#~and the two torsion angles $\phi,\psi$.\\
\-~~name~nn\_output\_1\\
\-~~torchann~\{\\
\-~~~~modelFile~model.pt\\
\-~~~~output\_component 0\\
...
\item %
  ...
  \key
    ...
    \emph{This option is currently only supported in NAMD: when enabled, Colvars-based atom selections may not be used.}
  }
}
...
\item %
  ...
  \key
    ...
    \emph{This option is supported both in NAMD or VMD:}
    its vaslue specifies the numeric index of the volumetric map to use for this collective variable component.
    The selected map must have already been loaded in \MDENGINE{}.
    \cvnamdonly{This keyword is mutually exclusive with \refkey{mapName}{colvar|mapTotal|mapName}: if the latter is given, the numeric ID is obtained implicitly from NAMD.}
  }
...
\item %
  ...
  \key
    ...
    filename}{%
    This option is mutually exclusive with \refkey{mapID}{colvar|mapTotal|mapID}\cvnamdonly{and \refkey{mapName}{colvar|mapTotal|mapName}}; if given, the map will be loaded internally in Colvars, and a Colvars-based atom selection should be provided with \refkey{atoms}{colvar|mapTotal|atoms}.
    \cvnamdonly{When this option is enabled in NAMD, the \texttt{MGridForceFile} keyword may not be used to select atoms.}
  }
...
\item %
  ...
  \key
    ...
    \cvvmdonly{In VMD, this definition is \emph{required}.}
    \cvnamdonly{In NAMD, this keyword is optional when maps are loaded using the \href{https://www.ks.uiuc.edu/Research/namd/2.14/ug/node43.html}{MGridForcePotFile} keyword.
...
\cvnamdonly{\textbf{Note:} in NAMD, the default value of this keyword is given by the \texttt{globalMasterFrequency} keyword, which also sets the lowest possible value.}
...
ref.\ \citealp{Darve2008}. For details about this implementation,
...
Alternately, grid parameters can be set directly inside the \texttt{abf} block by inserting a \texttt{grid} sub-block, as explained in \ref{sec:colvarbias_grid}.
...
is described in detail in ref.\ \citealp{Lesage2017}.
...
\cvsubsec{Adiabatic Bias Molecular Dynamics (ABMD)}{sec:colvarbias_abmd}
...
For simplicity, the case of an increasing value is described below, but enhancing downward motion of the variable is also supported via the \refkey{decreasing}{abmd|decreasing} flag.
...
\begin{itemize}
  \item %
    \key{forceConstant}{\texttt{abmd}}{%
      Force constant for the half-harmonic bias}
    {positive decimal}
    {The value of $k$. Stronger values result if faster motion of the coordinate, along more biased pathways.}

  \item \key{stoppingValue}{\texttt{abmd}}{%
     Value beyong which the reference value is not updated}
   {decimal}
   ...
   \item
   \labelkey{abmd|decreasing}
   \keydef{decreasing}{\texttt{abmd}}{%
   Should the variable decrease over time?}
    {boolean}
    {no}
...
\begin{itemize}
  \item \dupkey{name}{\texttt{abmd}}{sec:colvarbias}{biasing and analysis methods}
  \item \dupkey{colvars}{\texttt{abmd}}{sec:colvarbias}{biasing and analysis methods}
  \item \dupkey{outputEnergy}{\texttt{abmd}}{sec:colvarbias}{biasing and analysis methods}
  \item \dupkey{outputFreq}{\texttt{abmd}}{colvarbias|outputFreq}{biasing and analysis methods}
\end{itemize}
...
Alternately, grid parameters can be set directly inside the \texttt{metadynamics} block by inserting a \texttt{grid} sub-block, as explained in \ref{sec:colvarbias_grid}.
...
\textbf{Note:} usage of well-tempered metadynamics in combination with multiple walkers (\ref{sec:colvarbias_meta_mr}) is discouraged.  Should you want to use the two features, keep in mind that in Colvars, the well-tempered scaling factor is applied independently of the communication between replicas, i.e.\ each walker has its Gaussian hills scaled independently of the other walkers.
...
\cvsubsec{On-the-fly probability enhanced sampling (OPES)}{sec:colvarbias_opes}
...
Similarly to \texttt{metadynamics}, \texttt{opes{\textunderscore}metad} optimizes the bias on-the-fly, with a given \refkey{newHillfrequency}{opes_metad|newHillFrequency}.
...
If the exploration mode (keyword \refkey{explore}{opes_metad|explore}) is on, then the on-the-fly target probability distribution $p^{\mathrm{WT}}(\boldsymbol{\xi})$ is used to define the biasing energy:
...
The implementation of \texttt{opes{\textunderscore}metad}\ and its documentation are largely based on the \href{https://www.plumed.org/doc-master/user-doc/html/_o_p_e_s__m_e_t_a_d__e_x_p_l_o_r_e.html}{OPES module} of the \href{https://www.plumed.org/}{PLUMED} package.
...
\begin{itemize}
\item \dupkey{name}{\texttt{opes{\textunderscore}metad}}{sec:colvarbias}{biasing and analysis methods}
\item \dupkey{colvars}{\texttt{opes{\textunderscore}metad}}{sec:colvarbias}{biasing and analysis methods}
\item \dupkey{outputEnergy}{\texttt{opes{\textunderscore}metad}}{sec:colvarbias}{biasing and analysis methods}
\item %
  \labelkey{opes_metad|barrier}
  \key
    {barrier}{%
...
\item %
  \labelkey{opes_metad|newHillFrequency}
  \key
    {newHillFrequency}{%
    \texttt{opes{\textunderscore}metad}}{%
    Frequency of hill creation}{%
...
\item %
  \labelkey{opes_metad|biasfactor}
  \keydef
    {biasfactor}{%
    \texttt{opes{\textunderscore}metad}}{%
...
\item %
  \labelkey{opes_metad|sigma}
  \key
    {gaussianSigma}{%
    \texttt{opes{\textunderscore}metad}}{%
    The initial widths of the kernels (divided by the square root of gamma in exploration mode)}{%
    positive decimal}{%
    This option sets the initial $\sigma$ values of the Gaussian kernels. The number of \texttt{gaussianSigma} values should match the number of CVs used by \texttt{colvars}. If \refkey{adaptiveSigma}{opes_metad|adaptive_sigma} is set, then this option can be ignored.}
\item %
  \labelkey{opes_metad|sigma_min}
  \key
    {gaussianSigmaMin}{%
    \texttt{opes{\textunderscore}metad}}{%
...
\item %
  \labelkey{opes_metad|compression_threshold}
  \keydef
...
\item %
  \labelkey{opes_metad|adaptive_sigma}
  \keydef
    {adaptiveSigma}{%
    \texttt{opes{\textunderscore}metad}}{%
...
\item %
  \labelkey{opes_metad|adaptive_sigma_stride}
  \keydef
    {adaptiveSigmaStride}{%
    \texttt{opes{\textunderscore}metad}}{%
...
\item %
  \labelkey{opes_metad|nlist_pace_reset}
  \keydef
...
\item %
  \labelkey{opes_metad|nlist_param}
  \keydef
...
\item %
  \labelkey{opes_metad|fixed_sigma}
  \keydef
    {fixedSigma}{%
    \texttt{opes{\textunderscore}metad}}{%
...
\item %
  \labelkey{opes_metad|recursive_merge}
  \keydef
    {recursiveMerge}{%
    \texttt{opes{\textunderscore}metad}}{%
...
\cvsubsubsec{Multiple-walker options}{sec:colvarbias_opes_multiple_walker_options}

The following options can be used for the multiple-walker OPES, and require the direct communication between replicas through MPI\cvnamdonly{ or a Charm++ platform that supports partitions}:
\begin{itemize}
\item %
  \labelkey{opes_metad|multipleReplicas}
  \keydef
    {multipleReplicas}{%
    \texttt{opes{\textunderscore}metad}}{%
...
\item %
  ...
  \keydef
    ...
    string}{%
    replica index)}{%
    If \refkey{multipleReplicas}{opes_metad|multipleReplicas} is \texttt{on}, this option sets a unique identifier for this replicas.
    Specifying this option is thus only required when the replicas are launched as independent computations:
    when the replicas share a common parallel communication framework (i.e.\ they are all launched together as a single message-passing computation via MPI\cvnamdonly{or Charm++}) the default value of this keyword is the replica's numeric index (zero-based).
}
\item
  \labelkey{opes_metad|sharedFreq}
  \keydef{sharedFreq}{\texttt{opes{\textunderscore}metad}}{%
    Frequency (in number of steps) at which force samples are synchronized among the replicas}
  {positive integer or zero}
  {\refkey{outputFreq}{colvarbias|outputFreq}}
  {
    Every \texttt{sharedFreq} steps, the replicas communicate the samples that have been gathered since the last synchronization time.
...
\cvsubsubsec{Output options}{sec:colvarbias_opes_output_options}

The following options are used for setting the PMF and trajectory output. The PMF is calculated based on reweighting the CVs specified by \refkey{pmfColvars}{opes_metad|pmf_colvars}, which collects on-the-fly the biasing energy $V(\boldsymbol{\xi})$ and the values of CVs of every step, and builds a weighted histogram to calculate the unbiased probability of $\boldsymbol{\xi}$
\begin{equation}
...
The grid used for discretizing the free energy surface (PMF) computated by OPES is defined by the relevant options of the colvars.
Alternately, grid parameters can be set directly inside the \texttt{opes\_metad} block by inserting a \texttt{grid} sub-block, as explained in \ref{sec:colvarbias_grid}.

\begin{itemize}
\item %
  \labelkey{opes_metad|pmf}
  \keydef
    {pmf}{%
    \texttt{opes{\textunderscore}metad}}{%
    Output the PMF}{%
...
\item %
  \labelkey{opes_metad|pmf_colvars}
  \key
    {pmfColvars}{%
    \texttt{opes{\textunderscore}metad}}{%
    The CVs used for estimating the PMF}{%
    positive decimal}{%
    This option sets the CVs used for the PMF. The CVs specified in this option must have appeared in \refkey{colvars}{colvarbias|colvars} and had the boundaries and widths.}
\item %
  \labelkey{opes_metad|pmf_hist_freq}
  \keydef
    {pmfHistoryFrequency}{%
    \texttt{opes{\textunderscore}metad}}{%
    ...
    positive integer}{%
    0}{%
    If this option is larger than 0 and \refkey{pmf}{opes_metad|pmf} is on, then every \texttt{pmfHistoryFrequency} steps the current PMF will also be saved into ``\outputName\texttt{.colvars.$<$name$>$.$<$replicaID$>$.hist.pmf}''.}
\item %
  \labelkey{opes_metad|pmf_shared}
  \keydef
    {pmfShared}{%
    \texttt{opes{\textunderscore}metad}}{%
...
\item %
  \labelkey{harmonic|targetNumStages}
  \keydef
    {targetNumStages}{%
    \texttt{harmonic}}{%
...
The force constant of the harmonic restraint may also be changed over time, either to introduce or remove a restraint smoothly, or to estimate a restraint free energy\cite{Deng2009}.
...
\item %
  ...
  \key
    ...
    determination of a free energy profile for the restrained coordinate.\cite{Deng2009}}
...
\key
  {targetLowerWalls}{%
  \texttt{harmonicWalls}}{%
  Change the lower wall value(s) towards these values}{%
  positive decimal}{%
  This keyword allows changing the lower wall position over time.
  By default, this change is continuous over \texttt{targetEquilSteps}. In that case, the non-equilibrium work performed can be obtained by enabling  \texttt{outputAccumulatedWork}.
  If \texttt{targetNumStages} is provided, the change happens in the given number of increments. See \refkey{targetNumStages}{harmonic|targetNumStages} for details.
}
...
\key
  {targetUpperWalls}{%
  \texttt{harmonicWalls}}{%
  Change the upper wall value(s) towards these values}{%
  positive decimal}{%
  Analogous to \texttt{targetLowerWalls}.
}
...
The grid used for discretizing the free energy computated by \texttt{reweightaMD} is defined by the relevant options of the colvars.
Alternately, grid parameters can be set directly inside the \texttt{reweightaMD} block by inserting a \texttt{grid} sub-block, as explained in \ref{sec:colvarbias_grid}.
...
\item The Torch library is required to support the \texttt{torchANN} variable type, which is conditionally compiled if a Torch library is present. \cvgromacsonly{In GROMACS, this is automatically handled by supplying \texttt{-DGMX\_NNPOT=TORCH} in the CMake command line.} In most engines, patching will be required to support this.
