...
When \refkey{useHillsReflection}{metadynamics|useHillsReflection} is inactive, the Colvars metadynamics code keeps an explicit copy of each hill that crosses a grid boundary, using it to compute metadynamics forces outside the grid. While this approach slows down the simulation and increases state file's size over time, it helps maintain the accuracy and stability of the simulation. This feature is active unless \refkey{useHillsReflection}{metadynamics|useHillsReflection} is \texttt{on}, or when 'natural' boundaries or explicit \refkey{hardLowerBoundary}{colvar|hardLowerBoundary} and \refkey{hardUpperBoundary}{colvar|hardUpperBoundary} flags are set. If \refkey{useHillsReflection}{metadynamics|useHillsReflection} is \texttt{off}, it's recommended to use \texttt{harmonicWalls} restraints (see \ref{sec:colvarbias_harmonic_walls}), placed at least 8 Gaussian sigmas within the boundaries, to minimize inefficiencies from explicit hills outside the grid. These measures however do not prevent systematics errors building up at the boundaries, which can be only avoided by setting \refkey{useHillsReflection}{metadynamics|useHillsReflection} to \texttt{on}
