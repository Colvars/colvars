...
grids are big, whether for accuracy or because of the dimensionality curse filling them accurately will be long. In order to reduce
the time taken, we use kernel grid to leverage the free energy function regularity. At each time step $t$ : the system is in
...
the kernel's bandwidth $\sigma$ to a value, if the value is > 0 kernel grid is in use.
