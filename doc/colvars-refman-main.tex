...
values for each collective variable and in the last column the value of the function. The multicolumn text format 
 for a multidimensional function, f($\bm{\xi}_{1}$, $\bm{\xi}_{2}$, ..., $\bm{\xi}_{n}$), looks like:
    ...
    If the variable $\bm{\xi}_{i}$ is periodic then its \texttt{lowerBoundary} coincides with $\bm{\xi}^{1}_{i}$ and its
    minimum and maximum values on the multicolumn file are the edges of the periodicity interval. An example of one dimensional multicolumn text file is the following:
