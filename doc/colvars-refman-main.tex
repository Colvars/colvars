...
\begin{equation}
  ...
  V_t(\xi_t) = \left\{
    \begin{array}{l l}
      \frac{1}{2} k (\xi_t - \xi^{\mathrm{ref}}_t)^2 & \text{if }\ \xi_t < \xi^{\mathrm{ref}}_t \\
      0 & \text{otherwise}
    \end{array}
  \right.
\end{equation}

where $\xi^{\mathrm{ref}}_t$ is the high-water mark at time $t$, bounded by a user-defined stopping value $\xi^{\mathrm{stop}}$:
