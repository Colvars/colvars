...
\-(other~MDP~options)\\
\end{mdexampleinput}
When the \texttt{gmx grommp} command is called to create a TPR file, the contents of \texttt{my\_colvars\_config.txt} and of all files referenced by it are bundled in the TPR file as well.
...
When modifying the Colvars configuration (e.g.\ adding or removing a bias), a new TPR file should be built as well.
...
\item %
  \keydef{
    ...
    boolean}{%
    \texttt{no}}{%
    Activate Colvars computation in the current run.  Requires that the Colvars library was compiled with GROMACS, which is the default in a typical installation.
}
...
\item %
  \keydef
    ...
    string}{%
    colvars.dat}{%
    Name of the Colvars configuration file, which can be either an absolute path, or a path relative to the folder from which \texttt{gmx grompp} is called.
}
...
\textbf{Note:} \emph{This section describes how to use Colvars in GROMACS versions starting 2024, which support and include Colvars natively.}

To enable a Colvars-based calculation, just modify the \texttt{mdp} file to add those two keywords:
\begin{mdexampleinput}
  \-...\\
  \-colvars-active~=~yes\\
  \-colvars-configfile~=~my\_colvars.dat\\
  \-...\\
\end{mdexampleinput}

Then, create the \texttt{tpr} and launch the simulation as usual:
\begin{mdexampleinput}
  \-gmx~grompp~-f~system.mdp~-p~system.top~-c~init.gro~-o~test.tpr
  \-gmx~mdrun~-s~test.tpr~-deffnm~md~
\end{mdexampleinput}

Restarting from a previous simulation is done using the \texttt{-cpi} parameter for the checkpoint file (e.g.{} ``\texttt{state.cpt}''). This file holds the required information to restart the Colvars-based simulation.
\begin{mdexampleinput}
  \-gmx~mdrun~-s~test\_restart.tpr~-deffnm~md~-cpi~state.cpt
\end{mdexampleinput}

For the output files, the prefix of Colvars output files share the same prefix as the \texttt{edr} file, which is ``\texttt{ener}'' by default. This is modified if the \texttt{-defnnm} or the \texttt{-e} option of mdrun is used.
...
Colvars state files are written in binary format by setting the environment variable ``\texttt{COLVARS\_BINARY\_RESTART}'' to 1.
...
\cvlammpsonly{
LAMMPS controls the availability of unwrapped coordinates using the \texttt{unwrap} keyword in \ref{sec:colvars_mdengine_parameters}, which defaults to \texttt{yes}.
...
\cvgromacsonly{
In GROMACS, individual atom coordinates are systematically wrapped into a single unit cell, however Colvars maintains an unwrapped copy of the relevant coordinates to preserve their relative positions as provided in the input configuration.
These unwrapped coordinates are preserved across restarts from a checkpoint (\texttt{cpt}) file. However, whenever preparing a new simulation input with \texttt{gmx grompp}, users should provide input coordinates where the atoms involved in collective variables are not artificially moved across the boundary conditions, but occupy their relevant relative positions -- usually the nearest ones.
}
...
\noindent{}An up-to-date version of this manual for the latest version of Colvars may be accessed %
\cvgromacsonly{\href{https://colvars.github.io/\cvreleaselabel/colvars-refman-gromacs.html}{here}}%
\cvlammpsonly{\href{https://colvars.github.io/\cvreleaselabel/colvars-refman-lammps.html}{here}}%
\cvnamdonly{\href{https://colvars.github.io/\cvreleaselabel/colvars-refman-namd.html}{here}}%
\cvvmdonly{\href{https://colvars.github.io/\cvreleaselabel/colvars-refman-vmd.html}{here}}.
...
\item Scripting commands using the Tcl language (\url{https://www.tcl.tk}) are supported in VMD and NAMD.  All precompiled builds of each code include Tcl, and it is highly recommended to enable Tcl support in any custom build, using precompiled Tcl libraries from the UIUC website.
