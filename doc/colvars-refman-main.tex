...
When the initial configuration is provided using \texttt{colvarsConfig}, \texttt{colvarsInput} can be used to give the name of a state file to be read immediately at startup.
Once the Colvars module is initialized, a state file may be loaded dynamically at any time with the Tcl command \texttt{cv load}.
