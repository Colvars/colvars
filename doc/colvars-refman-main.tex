...
\noindent\texttt{gmx mdrun -s test.tpr -colvars variables.colvars.dat biases.colvars.dat}

Restarting from a previous simulation is done using the \texttt{-colvars\_restart} parameter:\\
\noindent\texttt{gmx mdrun -s test\_restart.tpr -cpi state.cpt -colvars config.colvars.dat
 -colvars\_restart md.colvars.state.dat}

Note that the standard extension of restart files ``\texttt{.colvars.state}'' has to be changed manually to end with  ``\texttt{.dat}'' to satisfy Gromacs requirements on filenames.
}
