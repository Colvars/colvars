...
Some colvar components have default values, expressed in \AA{}ngstr\"om in this documentation. They are converted to the current length unit as needed.
...
All of the above keywords except for the name of the configuration file may also be given (or overriden) using \texttt{fix\_modify}, as well as new Colvars configuration files (see \ref{sec:cv_scripting} for more details).
...
\cvlammpsonly{It is possible to use \texttt{fix\_modify} on a previously created instance of \texttt{fix colvars} is possible at any time.
However, keep in mind that this fix is only fully initialized at the beginning of the first simulation step.
Only the variables and biased created during the previous \texttt{run} or \texttt{minimize} command will be accessible, even if a Colvars configuration file that defines them has already been given.
Should you want to trigger immediate initialization of Colvars, please consider running ``\texttt{run 0}'' in the LAMMPS script.
}
...
The most frequent commands of the Colvars scripting interface are discussed and exemplified in this section.
\ifdefined\cvscriptcmdapi{For a full list of scripting commands/subcommands available through the \ifdefined\cvvmdornamd{``\texttt{cv}'' command}\fi\cvlammpsonly{``\texttt{fix\_modify}'' command}, see section~\ref{sec:cvscript_cmdline}.}\fi
...
How the grid's boundaries affect the sequence of points depends on how the contents of the file were computed.  In many cases, such as histograms and PMFs computed by metadynamics (\ref{sec:colvarbias_ebmeta}), the values of $\xi_i$ in the first few columns correspond to the \emph{midpoints} of the corresponing bins, i.e.\ $\xi^{1}_{1} = \mathtt{min}(\xi_{i}) + \mathtt{width}(\xi_{i})/2$.
...
\begin{equation}
  \label{eq:cvc_multi_map}
    \xi(\mathbf{X}) = \sum_{k=1}^{K} k \Phi_{k}(\mathbf{X}) = \sum_{k=1}^{K} k \sum_{i=1}^{N}\phi_{k}(\mathbf{x}_{i})
\end{equation}
...
\begin{equation}
  ...
  V(\xi) = k \left(\frac{\xi - \xi_0}{w_{\xi}}\right)
\end{equation}
...
\cvurl{https://www.plumed.org/doc-v2.6/user-doc/html/sum_hills.html}\\
...
\cvgromacsonly{\cvurl{https://colvars.github.io/colvars-refman-gromacs/colvars-refman-gromacs.html}}
