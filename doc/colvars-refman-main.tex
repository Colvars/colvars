...
\item %
  \keydef
    ...
    A value must be defined and is set by default, even when Colvars embeds its state information in a LAMMPS restart files, to allow the output of other files (see \ref{sec:colvars_output}).
}
...
\cvlammpsonly{
Each \texttt{fix\_modify} command may be used in one of two ways:
\begin{enumerate}
