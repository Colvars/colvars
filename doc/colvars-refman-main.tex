...
\-\}
\end{cvexampleinput}
This version results implementing extremely fast updates.
} % cvvmdonly dashboard
...
\cvalchlambdaonly{
\item \refkey{alchLambda}{colvar|alchLambda}
\item \refkey{alchFlambda}{colvar|alchFlambda}
}
\end{itemize}
...
\cvsubsec{Dense neural network in CV space}{sec:cvc_neuralNetwork}

This colvar computes a non-linear combination of other scalar colvar components, and the non-linear transformation is defined by a dense neural network.

\begin{figure}[!ht]
	\centering
	\cvnamdugonly{\includegraphics[width=9cm]{figures/dense_nn}}
	\cvvmdugonly{\includegraphics[width=9cm]{pictures/dense_nn}}
	\cvrefmanonly{\ifdefined\HCode{\HCode{<img class="neural_network" src="dense_nn.png" alt="Dense neural network">}}\else\includegraphics[width=9cm]{dense_nn}\fi}
...
\cvsubsubsec{\texttt{NeuralNetwork}: Dense neural network.}{sec:cvc_neuralNetwork}
...
{output\_component}{%
	...
	specify the output node to be used as the value of this CV.
}
...
{layer$i$\_WeightsFile}{%
	\texttt{NeuralNetwork}}{%
	The weights of layer $i$}{%
	UNIX filename}{%
	\texttt{layer1\_WeightsFile}, \texttt{layer2\_WeightsFile}, $\cdots$, \texttt{layer$i$\_WeightsFile} are used to specify the weights of the first, second, $\cdots$, $i$-th layers, respectively. The weights are stored in a plain text file, where the number at $k$-th row and $l$-th column represents the weight of $k$-th (output) node at $i$-th layer with respect to the $l$-th (input) node at $i-1$ layer.
}
...
{layer$i$\_BiasesFile}{%
	\texttt{NeuralNetwork}}{%
	The biases of layer $i$}{%
	UNIX filename}{%
	\texttt{layer1\_BiasesFile}, \texttt{layer2\_BiasesFile}, $\cdots$, \texttt{layer$i$\_BiasesFile} are used to specify the biases of the first, second, $\cdots$, $i$-th layers, respectively. The biases are stored in a plain text file with only one column, where the number at $k$-th row represents the bias of $k$-th (output) node at $i$-th layer.
}
...
{layer$i$\_activation}{%
	\texttt{NeuralNetwork}}{%
	The activation function of layer $i$}{%
	\texttt{tanh}, \texttt{linear} or \texttt{sigmoid}}{%
	Select the activation function for $i$-th layer. Available choices for activation function are \texttt{tanh}, \texttt{linear} and \texttt{sigmoid}.
}
...
{layer$i$\_custom\_activation}{%
	\texttt{NeuralNetwork}}{%
	An alternative custom expression as the activation function of layer $i$}{%
	string}{%
	Mathematical expression to define the activation function of layer $i$. The 
	input value must be written as \texttt{x}. For example, the ELU activation 
	function can be expressed as \texttt{select(step(x), alpha*(exp(x)-1), x)}. 
	It should be noted that this option is mutually exclusive with the option 
	\refkey{layer$i$\_activation}.
}
...
\item %
  ...
  \key{scaledBiasingForceFactorsGrid}{colvar bias}{A histogram file with the scaling factor of biasing force in each bin}{string}{%
    If \texttt{scaledBiasingForce} is set to \texttt{on}, this option accepts the filename of the histogram file that contains the scaling factors. The format of the histogram file is the same as the PMF files written by \texttt{metadynamics} or \texttt{ABF}.
  }
