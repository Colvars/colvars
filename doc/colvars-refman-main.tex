...
\noindent\texttt{gmx mdrun -s test.tpr -deffnm md -colvars variables.colvars.dat biases.colvars.dat}\\

\noindent{}Restarting from a previous simulation is done using the \texttt{-colvars\_restart} parameter and the Gromacs checkpoint file (``\texttt{state.cpt}'').
Both file hold information to restart properly the simulation.\\
\noindent\texttt{gmx mdrun -s test\_restart.tpr -deffnm md -cpi state.cpt -colvars config.colvars.dat
 -colvars\_restart md.colvars.state.dat}\\

\noindent{}Note that the standard extension of all files have to be changed manually to end with  ``\texttt{.dat}'' to satisfy Gromacs requirements on filenames.

\noindent{}For the output files, the prefix of colvars output files will use the \texttt{-defnnm} option of mdrun, or be the same as the Gromacs log file if the option is not set. Otherwise, the prefix will be \emph{output}.
...
$N$\\
\multicolumn{4}{l}{\texttt{Comment line}}\\
$E_{1}$ & $x_{1}$ & $y_{1}$ & $z_{1}$\\
$E_{2} $ & $x_{2}$ & $y_{2}$ & $z_{2}$\\
\ldots\\
$E_{N}$ & $x_{N}$ & $y_{N}$ & $z_{N}$ \\
...
\cvvmdonly{Note that the coordinates read by Colvars internal parser are stored as double-precision floating point numbers (unlike VMD's reader, which uses single precision).}
...
For details on how to optimize numerical accuracy and parallel performance, as well as automating the creation of suitable input decks, see \href{https://colvars.github.io/multi-map/multi-map.html}{this page}.
A generic example of Colvars configuration is also reported below.
