...
\item %
  \labelkey{colvarbias|zeroStepData}
  \keydef
    {zeroStepData}{%
    colvar bias}{%
    ...
    boolean}{%
    \texttt{off}}{%
    This option is meaningful for biases that record data during a simulation, sh as ABF (\ref{sec:colvarbias_abf}), metadynamics (\ref{sec:colvarbias_meta}), histograms (\ref{sec:colvarbias_histogram}) and in general any bias that accumulate samples from thermodynamic integration (\ref{sec:colvarbias_ti}).
    % TODO: unclear how to handle ALB at the moment
    When this option is disabled (default) the bias's properties such as energy and forces will be computed, but data will only be recorded after the first coordinate update: this is generally the correct choice in simulation runs.
    Optionally, this option may be turned on to support custom algorithms\cvnamdonly{; when updating the bias's internal data requires data from previous steps (for example, total atomic forces) turning on this flag will raise an error.}
  }
...
\item \dupkey{zeroStepData}{\texttt{abf}}{sec:colvarbias}{biasing and analysis methods}
...
\item \dupkey{zeroStepData}{\texttt{metadynamics}}{sec:colvarbias}{biasing and analysis methods}
...
\item \dupkey{zeroStepData}{\texttt{harmonic}}{sec:colvarbias}{biasing and analysis methods}
...
\item \dupkey{zeroStepData}{\texttt{harmonicWalls}}{sec:colvarbias}{biasing and analysis methods}
...
\item \dupkey{zeroStepData}{\texttt{histogram}}{sec:colvarbias}{biasing and analysis methods}
% \item \dupkey{outputEnergy}{\texttt{histogram}}{sec:colvarbias}{biasing and analysis methods}

\item \dupkey{outputFreq}{\texttt{histogram}}{colvarbias|outputFreq}{biasing and analysis methods}
