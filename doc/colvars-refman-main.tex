...
The method is designed so that during the simulation the resulting distribution  the collective variable $\bm{\xi}$ converges to $\rho_{exp}(\bm{\xi})$. 
...
\item %
  ...
  \key
    ...
    To avoid divisions by zero (see eq.{} \ref{eq:colvars_ebmeta_pot}), values of $\rho_{exp}$ equal to zero are replaced by the smallest positive value read in the same file.
...
\item %
  \key
    ...
    positive integer}{%
    The EBMetaD approach may introduces large hills in regions with small values of the target probability distribution (eq.{} \ref{eq:colvars_ebmeta_pot}). 
