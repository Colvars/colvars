...
\cvsubsubsec{\texttt{eulerPhi}: Row angle from 
...
this case, the complete rotation described by \texttt{orientation} can 
optionally be decomposed into three Euler angles: (i) $\phi$, the rotation 
angle from the \textit{x} axis to the \textit{N} axis, (ii) $\theta$, the 
rotation angle from the \textit{z} axis to the \textit{Z} axis and (iii) 
$\psi$, the rotation angle from the \textit{N} axis to the \textit{X} axis.
