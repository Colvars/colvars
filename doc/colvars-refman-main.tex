...
\textbf{Tip:} The exploration of unphysically low values of the target distribution (which would lead to very large hills and possibly numerical instabilities) can be prevented by restricting sampling to a given interval, using e.g.{} \texttt{harmonicWalls} restraint (\ref{sec:colvarbias_harmonic_walls}).

As with standard metadynamics, multiple probability distributions on different variables can be targeted simultaneously introducing multiple independent \texttt{metadynamics} blocks in the input.
Multidimensional probability distributions can be targeted instead using a single \texttt{metadynamics} block using multiple \texttt{colvars} and a multidimensional target distribution file (see \ref{sec:colvar_multicolumn_grid}).
