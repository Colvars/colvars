...
This parameterization is mathematically well defined for almost all rotations, including very small ones, such as those arising when the current coordinates are almost completely aligned with the reference ones.
...
For these reasons, \refkey{tilt}{colvar|tilt} and \refkey{spinAngle}{colvar|spinAngle} are useful to characterize \emph{small rotations} via a reduced number of degrees of freedom (two instead of three).
