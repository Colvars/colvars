...
\begin{itemize}
 ...
 \item Using the ful-featured Tcl scripting interface as documented in section~\ref{sec:cv_scripting}; see in particular the example in section~\ref{sec:cv_command_example}.
\end{itemize}
}
...
\cvvmdonly{Writing the configuration for a collective variable in VMD is made much easier using the Dashboard and its configuration editor (section~\ref{sec:dashboard_config_editor}), however it does not handle biases.}
...
\cvsubsec{Dashboard window}{sec:dashboard_main}
...
The configuration editor can be started with the ``Edit'' button.
If one or several variables selected, their configuration is loaded, those variables will be replaced when applying the new configuration.
...
\cvsubsec{Plotting collective variable trajectories}{sec:dashboard_plots}

Timeline plots show the selected variables as a function of time. A vertical bar indicates the current frame. it can be changed either using VMD's trajectory animation controls, or directly in the plot window by clicking the mouse inside the graph, or using the keyboard left/right arrows.
...
\cvsubsec{Visualization tools}{sec:dashboard_visualization}
