...
this case, as illustrated in Figure \ref{fig:eulerangles}, the complete 
rotation described by \texttt{orientation} can optionally be decomposed into 
three Euler angles: (i) $\phi$, the rotation angle from the \textit{x} axis to 
the \textit{N} axis, (ii) $\theta$, the rotation angle from the \textit{z} axis 
to the \textit{Z} axis and (iii) $\psi$, the rotation angle from the \textit{N} 
axis to the \textit{X} axis.
\begin{figure}
\begin{center}
\includegraphics[width=0.5\textwidth]{eulerangles}
\end{center}
\caption{\label{fig:eulerangles}Schematic representation of the euler angles. 
The roll, pitch and yaw angles are shown as $\phi$, $\theta$ and $\psi$, 
respectively.}
\end{figure}

Although the use of Euler angles to characterize three-dimensional spatial 
rotation is more straightforward than Quaternions, one should pay attention to 
the possible gimbal lock problem in practice. A typical application of Euler 
angles is describing the orientational degrees of freedom in protein-ligand 
binding free-energy calculations\cite{Fu2017} (see also the tutorial 
\url{http://www.ks.uiuc.edu/Training/Tutorials/namd/PLB/tutorial-protein-ligand.pdf}
).

This component accepts exactly the same options as \texttt{orientation}, and 
measures the rotation angle from \textit{x} axis to the \textit{N} axis. This 
