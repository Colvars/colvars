...
{\texttt{no}}{
  ...
  integration. This means solving the following weighted Poisson equation:
    \begin{equation}
      \nabla\cdot \theta \nabla A_t = \nabla \cdot G_t.
    \end{equation}
  With $\theta$, the weights, corresponding to how much an area has been visited during the simulation. Doing so, the
  most visited cells which corresponds to the cells where the gradient is the best known have more importance in the
