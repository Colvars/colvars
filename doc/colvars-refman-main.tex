...
If this flag is \texttt{on}, the default value of \texttt{normalizeVector} is also set to \texttt{on}, resulting in $\xi = 0$ when the atoms are at the set of positions ``A'' and $\xi = 1$ at the set of positions ``B''.
}
\end{cvcoptions}
This component returns a number (in \lengthunit), whose value ranges between
