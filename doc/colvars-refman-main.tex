...
\item %
  \keydef
    ...
    may use the scripting interface (see \ref{sec:scripting}) to access
...
the component \texttt{orientation}: \texttt{atoms} and \texttt{refPositions}\cvnamebasedonly{, or \texttt{refPositionsFile}, \texttt{refPositionsCol} and \texttt{refPositionsColValue}}.
The returned value is the cosine of the angle of rotation $\omega$ between the current and the reference positions.
...
\item%
  \keydef
    ...
    colvar is a scalar or one of the following vector types: \texttt{vector}
    (a 3D vector), \texttt{unitvector} (a normalized 3D vector), or
    \texttt{unitquaternion} (a normalized quaternion). Non-scalar values
    must be formatted as parenthesized, comma-separated lists, e.g. ``(1., 2., 3.)'',
    for proper parsing.}
\end{itemize}
} % \cvscriptonly
...
\cvscriptonly{\cvsec{Colvars scripting}
\label{sec:scripting}

Collective variables and biases can be added, queried and deleted through the scripting command
\cvnamdonly{\texttt{cv}}\cvvmdonly{\texttt{colvars}}, with the following syntax: \cvnamdonly{\texttt{cv}}\cvvmdonly{\texttt{colvars}}\texttt{~$<$subcommand$>$ [args...]}
for which the available subcommands are listed below.
\cvnamdonly{This interface is particularly useful to implement custom biases as scripted colvar forces.
See the \texttt{scriptedColvarForces} option in \ref{sec:colvars_config}.}
