...
\item %
  ...
  \key
    ...
    The model is assumed to take one-dimensional tensor (excluding batch dimension) as input.
  }
...
\item %
  ...
  \keydef
    ...
    0-based integer}{%
    0}{%
    This option specifies the index of the output node to be used as the value of collective variable (see also the equivalent keyword \refkey{output\_component}{colvar|NeuralNetwork|output_component} in the built-in neural network implementation).
  }
...
\begin{cvexampleinput}
\-colvar~\{\\
\-~~\#~define a scalar CV as $\xi=f_1(x_2,y_2,z_2,x_4,y_4,z_4, \phi, \psi)$, where\\
\-~~\#~the map $f_1: \mathbb{R}^8\rightarrow \mathbb{R}$ is defined by the first component of the Torch model,\\
\-~~\#~$(x_2,y_2,z_2)$ and $(x_4, y_4, z_4)$ are coordinates of the atoms with\\
\-~~\#~atomNumbers $2$ and $4$, respectively, and $\phi,\psi$ are two torsion angles.\\
\-~~name~nn\_output\_1\\
\-~~torchann~\{\\
\-~~~~modelFile~model.pt\\
\-~~~~output\_component 0\\
