...
The configuration will contain two blocks, one defining the distance \texttt{colvar} and the other, the moving \texttt{harmonic} restraint.
...
Reading this input in plain English: the variable named \texttt{dist} consists in a distance function between the centers of two groups: the ligand (atoms 42 to 55) and the alpha carbon atoms (CA) of residues 15 to 30 in the protein (segment name PR).
To that variable, we apply a \texttt{harmonic} potential of force constant 20~\cvnamdonly{kcal/mol}\cvvmdonly{kcal/mol}\cvlammpsonly{energy units}/\AA$^2$, initially centered around a value of 4~\AA, which will increase to 15~\AA{} over 500k MD timesteps.
...
\cvsubsec{Configuration syntax for the Colvars module}
...
\item %
  \keydef
    ...
    boolean}{%
    \texttt{on}}{%
    If this flag is enabled (default), SMP parallelism over threads will be used to compute variables and biases, provided that this is supported by \MDENGINE{}.}
...
Below is an example configuration for an atom group with identifier ``\texttt{atoms}'', which uses an unusually varied combination of selection keywords:\\
...
\cvsubsec{Performance a Colvars calculation based on group size.}
...
\item %
  \keydef
    ...
    \cvnamdonly{It also defines the variable name representing the component's value in a \texttt{customFunction} expression.}}
...
\cvsubsubsec{Calculating total forces.}
...
\cvnamdonly{
\cvsubsec{Colvars as custom functions of components}
...
\label{sec:colvarbias_force_k_ti}
...
\texttt{histogram} block to the Colvars configuration file.
...
\cvsubsubsec{Grid definition for multidimensional histograms}
\label{sec:colvarbias_histogram_grid}
