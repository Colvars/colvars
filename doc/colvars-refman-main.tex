...
\cvsubsec{Computational cost of colvars calculations based on group size.}
\label{sec:colvar_atom_groups_scaling}

In parallel simulations (as supported by NAMD and LAMMPS), the calculation of energy and forces is distributed over many nodes and processor cores.
Unless the calculation of each colvar component is also parallelized, the overall performance may be affected.
You can check the output to see if the parallel calculation of a certain component is available: if the keyword \texttt{scalable} (see \ref{sec:cvc_common}) is \texttt{on}, this will be reported by the message ``Will enable scalable calculation for group \ldots'' for each of its groups.

If the calculation of a component is not parallelized (as reported by the message ``Scalable calculation is not available''), the performance of the first computational node may decrease significantly.
Even more importantly, additional communication will carried out between nodes to transfer the required atomic coordinates.
\cvnamdonly{The latency-tolerant design and dynamic load balancing of NAMD alleviate both factors, but a noticeable performance impact may be observed.}
...
This can be defined using the same options as the component \texttt{orientation}: \texttt{atoms} and \texttt{refPositions} or \texttt{refPositionsFile}\cvnamebasedonly{, \texttt{refPositionsCol} and \texttt{refPositionsColValue}}.
...
All types of components support the following keywords:
\begin{itemize}
\item %
  \keydef
    {name}{%
    ...
    The name is an unique case-sensitive string which allows the
    colvar module to identify this component. This is useful, for example,
...
\item %
  \keydef
    ...
    If set to \texttt{on} (default), the module will attempt to calculate this component in parallel,
    over multiple processor cores or multiple nodes (whichever is faster). 
    Whether this feature is available depends on the specific configuration and the hosting code:
    a message is printed to indicate this. 
    If available, this feature reduces the overhead of a collective variables-based calculation:
    otherwise, the component is calculated entirely on one processor core.
    This flag influences parallel performance, but does not affect numerical results: therefore, it should only
    be turned off for debugging or testing purposes.
  }
\end{itemize}
