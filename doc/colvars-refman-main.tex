...
\end{aligned}$\newline
    ...
    When \refkey{fittingGroup}{atom-group|fittingGroup} is enabled, the computation is a $O(n^{(\rm fit)}+n^{(\rm main)})$ loop for all variables, including RMSDs. \textbf{In general, this option should be always on, unless you are sure that the forces acting on the fitting group are zero.}
}
