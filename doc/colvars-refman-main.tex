...
\item %
  \key
    ...
    For each variable or bias currently defined, information is loaded only if it is available; conversely, information about variables or biases not currently defined is silently ignored.
    \emph{This check is done by matching the names of variables or biases: it is the user's responsibility to ensure that these are consistent between runs.}
    \cvnamdonly{This file can also be loaded with the Tcl command \texttt{cv load}.}
    \cvvmdonly{This file can also be loaded with the Tcl command \texttt{cv load}.}
  }
