...
This colvar computes a non-linear combination of other scalar colvar components, and the non-linear transformation is defined by a dense neural network.\cite{Chen2022}
...
\item %
  ...
  {Mathematical expression to define the activation function from layer $i-1$
   ...
   \texttt{select(step(x), alpha*(exp(x)-1), x)}. It should be noted that this option
