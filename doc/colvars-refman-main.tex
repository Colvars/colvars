...
Similarly to \texttt{metadynamics}, \texttt{opes{\textunderscore}metad} optimizes the bias on-the-fly, with a given \texttt{pace}.
...
V_n(\boldsymbol{\xi}) = -k_{\mathrm{B}}T(1-1/\gamma)\log\left(\frac{P_n(\boldsymbol{\xi})}{Z_n}+\epsilon\right),
\end{equation}
...
The \texttt{opes{\textunderscore}metad} in Colvars is largely based on the code of the OPES module in PLUMED \cite{tribello_plumed_2014}, except that it has the following limitations:
\begin{itemize}
...
\item Due to the same limitation of Colvars as descrbited above, you cannot collect samples on-the-fly by chaining \texttt{opes{\textunderscore}metad} with a weighted histogram for PMF calculations. Although the PMF calculation via reweighting is implemented as \texttt{pmf{\textunderscore}grid}, the functionality is quite limited. The CVs in \texttt{pmf{\textunderscore}colvars} must be the biased ones. In other words, you cannot apply bias on $(\xi_1, \xi_2)$ and compute the PMF along $\xi_3$ by the on-the-fly reweighting. You can only specify one set of CVs for estimating the PMF either (you cannot apply the OPES bias on $(\xi_1, \xi_2, \xi_3)$ and compute two 2D PMFs along $(\xi_1, \xi_2)$ and $(\xi_2, \xi_3)$ on-the-fly simultaneously using the \texttt{pmf{\textunderscore}grid}). However, you can always bypass this limitation by post-hoc reweighting with the trajectories.
\item Colvars computes the biases in parallel with SMP, so it cannot use multiple threads again for computing the probability and the normalization factor, searching the kernels that are able to compress and updating the neighbor list.
\end{itemize}
...
\item %
  ...
  \keydef
    {biasfactor}{%
    \texttt{opes{\textunderscore}metad}}{%
    The $\gamma$ biasfactor used for the well-tempered target distribution. Cannot be "inf" }{%
    positive decimal}{%
    $\mathrm{barrier} / k_{\mathrm{B}} T$}{%
    This option sets the $\gamma$ in Eq.(\ref{eq:opes_potential}).}
\item %
  ...
  \key
    ...
    positive decimal}{%
    This option sets the initial $\sigma$ values of the Gaussian kernels. The number of \texttt{sigma} values should match the number of CVs used by \texttt{colvars}. If \texttt{adaptive{\textunderscore}sigma} is set, then this option can be ignored.}
\item %
  \labelkey{opes_metad|sigma_min}
  \key
    {sigma{\textunderscore}min}{%
    \texttt{opes{\textunderscore}metad}}{%
    The minimum widths of the kernels, divided by the square root of gamma}{%
    positive decimal (optional)}{%
    This option sets the minimum $\sigma$ values of the Gaussian kernels. The number of \texttt{sigma{\textunderscore}min} values should match the number of CVs used by \texttt{colvars}. The OPES method scales down the $\sigma$ during the simulation, and will never shrink $\sigma$ under \texttt{sigma{\textunderscore}min} if this option is set.}
\item %
...
\item %
  ...
  \keydef
    {compression{\textunderscore}threshold}{%
    \texttt{opes{\textunderscore}metad}}{%
...
\item %
  ...
  \keydef
    {adaptive{\textunderscore}sigma{\textunderscore}stride}{%
    \texttt{opes{\textunderscore}metad}}{%
...
\item %
  ...
  \keydef
    {nlist{\textunderscore}pace{\textunderscore}reset}{%
    \texttt{opes{\textunderscore}metad}}{%
...
The following options are used for setting the PMF and trajectory output. The PMF is calculated based on reweighting the CVs specified by \texttt{pmf{\textunderscore}colvars}, which collects on-the-fly the biasing energy $V(\boldsymbol{\xi})$ and the values of CVs of every step, and builds a weighted histogram to calculate the unbiased probability of $\boldsymbol{\xi}$
\begin{equation}
...
\item %
  ...
  \keydef
    {pmf{\textunderscore}hist{\textunderscore}freq}{%
    \texttt{opes{\textunderscore}metad}}{%
    The frequency of saving the PMF history file}{%
    positive integer}{%
    0}{%
    If this option is larger than 0 and \refkey{pmf}{opes_metad|pmf} is on, then every \texttt{pmf{\textunderscore}hist{\textunderscore}freq} steps the current PMF will also be saved into ``\outputName\texttt{.colvars.$<$name$>$.$<$replicaID$>$.hist.pmf}''.}
\item %
  \labelkey{opes_metad|pmf_shared}
  \keydef
    {pmf{\textunderscore}shared}{%
    \texttt{opes{\textunderscore}metad}}{%
...
\item %
  ...
  \keydef
    {print{\textunderscore}trajectory{\textunderscore}frequency}{%
    \texttt{opes{\textunderscore}metad}}{%
    The frequency of saving the trajectories}{%
    positive integer}{%
    \refkey{colvarsTrajFrequency}{Colvars-global|colvarsTrajFrequency}}{%
    If this option is larger than 0, then every \texttt{print{\textunderscore}trajectory{\textunderscore}frequency}, the simulation time in ps, the values of CVs, the biasing energy, $c(t)$, $Z_n$ (if \refkey{no{\textunderscore}zed}{opes_metad|no_zed} is off), the effective sample size ($N_{\mathrm{eff}}$ in Ref.\cite{Invernizzi2020}), the work (if \refkey{calc{\textunderscore}work}{opes_metad|calc_work} is on), the total number of compressed kernels, the number of kernels in the neighbor list (if \refkey{nlist}{opes_metad|nlist} is on), and the number of steps from the last neighbor list update (if \refkey{nlist}{opes_metad|nlist} is on) will be written to
...
\-~~adaptive\_sigma~on                \\
\-~~nlist~on                          \\
\-~~print\_trajectory\_frequency~500  \\
\-~~pmf~on                            \\
\-~~pmf\_colvars~phi~psi              \\
\-~~pmf\_hist\_freq~1000              \\
\-~~outputEnergy~on                   \\
\-\}
\end{cvexampleinput}
