...
\-...\
\-colvars-active~=~yes\\
\-colvars-configfile~=~my\_colvars\_config.txt\\
\-...\\
\-(other~MDP~options)\\
\end{mdexampleinput}
...
\cvnamdonly{
 When PBCs are defined, NAMD maintains
  ...
  bonds).  However, different

the same treatment  condition may fail if the group spans
  different molecules

.  In that case, writing the NAMD output and restart files
