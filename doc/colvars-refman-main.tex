...
To enable a simulation with Colvars, one or more options should be added to the typical \texttt{mdrun} parameters, for example:
\begin{mdexampleinput}
\-;~MDP~file\\
\-...\
\-colvars-active~=~yes\\
...
\-(other~MDP~options)\\
\end{mdexampleinput}
When the MDP file is compiled into a TPR file, the contents of \texttt{colvars.inp} and of all files referenced by it are bundled in the TPR file as well.
...
\item %
  \keydef
    ...
    string}{%
    no}{%
    Name of the Colvars configuration file, which can be either an absolute path, or a path relative to the folder from which \texttt{gmx mdrun} is called.
}
