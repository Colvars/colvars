...
\# & $N_{\mathrm{cv}}$ & & & & \\
\# & $\mathtt{min}(\xi_{1})$ & $\mathtt{width}(\xi_{1})$ & $\mathtt{nbins}({\xi_{1}})$ & $\mathtt{periodic}({\xi_{1}})$ \\
\# & $\mathtt{min}(\xi_{2})$ & $\mathtt{width}(\xi_{2})$ & $\mathtt{nbins}({\xi_{2}})$ & $\mathtt{periodic}({\xi_{2}})$ \\
\# & \ldots & \ldots & \ldots & \ldots \\
\# & $\mathtt{min}(\xi_{N_{\mathrm{cv}}})$ & $\mathtt{width}(\xi_{N_{\mathrm{cv}}})$ & $\mathtt{nbins}({\xi_{N_{\mathrm{cv}}}})$ & $\mathtt{periodic}({\xi_{N_{\mathrm{cv}}}})$ & \\
\\
...
For each variable $\xi_{i}$, $\mathtt{min}(\xi_{i})$ is the lowest value sampled by the grid (i.e.{} the left-most boundary of the grid along $\xi_{i}$), $\mathtt{width}(\xi_{i})$ is the width of each grid bin along $\xi_{i}$, $\mathtt{nbins}(\xi_{i})$ is the number of bins and $\mathtt{periodic}(\xi_{i})$ is a flag whose value is 1 or 0 depending on whether the grid is periodic along $\xi_{i}$.
In most situations:
\begin{itemize}
...
\item $\mathtt{nbins}(\xi_{i})$ is calculated from the two above numbers and the \refkey{upperBoundary}{colvar|upperBoundary} keyword;
...
In this case, free-energy gradients are accumulated on an \texttt{(nbins)}-long grid along each variable $\xi$: after these gradients are integrated, the resulting PMF is binned on a grid that is \texttt{(nbins+1)}-long along $\xi$.
...
The target distribution file (``targetdist1.dat'', see below) is a text file in ``multicolumn'' format (\ref{sec:colvar_multicolumn_grid}).
For example, the following defines a one-dimensional histogram ranging from 0.0 to 100.0 with a spacing of 0.1:\\
\begin{tabular}{l l l l l l}
\\
\# & \texttt{1} & & & & \\
\# & \texttt{0.0} & \texttt{0.1} & \texttt{1000} & \texttt{0} \\
\\
& \texttt{0.05} & \texttt{4.2377E-74} \\
& \texttt{0.15} & \texttt{1.34471E-73} \\
& \ldots & \ldots\\
& \texttt{99.95} &  \texttt{1.35489E-79} \\
\\
\end{tabular}
\\
\noindent{}Note that in the target distribution file exemplified above, the values of the variable (first column) are 
