...
This is achieved by weighting the Gaussian functions used in the metadynamics approach by the inverse of the target probability distribution:
\begin{equation}
...
can be obtained by averaging (after an equilibration time) multiple time-dependent free energy estimates;
...
The following options define the configuration for the ensemble-biased metadynamics approach:
\begin{itemize}
\item %
...
\noindent{}The target distribution file (``targetdist1.dat'', see below) is a text file in ``multicolumn'' format (\ref{sec:colvar_multicolumn_grid}) with the same \texttt{width} (0.1 in this case):\\
\begin{tabular}{l l l l l l}
\\
\# & \texttt{1} & & & & \\
