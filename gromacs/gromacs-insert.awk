...
/static void pull_potential_wrapper/ {
    printf "static void colvars_potential_wrapper(FILE *fplog,t_commrec *cr, t_inputrec*ir, matrix box, rvec x[], ForceWithVirial *force, t_mdatoms *mdatoms,gmx_enerdata_t *enerd, gmx_int64_t step, gmx_wallcycle_t wcycle) {\nt_pbc pbc;\nwallcycle_start(wcycle, ewcPULLPOT);\nset_pbc(&pbc, ir->ePBC, box);\nenerd->term[F_COM_PULL] += colvars_potential(ir, mdatoms, &pbc, step, x, force);\nwallcycle_stop(wcycle, ewcPULLPOT);\n}\n\n"
}

# Insert the calls to colvars_potential_wrapper().
/inputrec->bPull && inputrec->pull->bPotential/ {
    printf "/* Colvars operation is currently defined by userint1*/\nif (0 != inputrec->userint1) colvars_potential_wrapper(fplog, cr, inputrec, box, x, forceWithVirial, mdatoms, enerd, step, wcycle);\n\n"
}
