...
int main(int argc, char *argv[])
{
  ...
  if (countfile.size()) {
    ...
    if (stat(countfile.c_str(), &buffer) == 0) {
      ...
      if (!count_ptr || count_ptr->nd == 0) {
        ...
        proxy->cvmodule->error("Error reading count grid.");
        return proxy->cvmodule->get_error();
      }
      if (debug)
  ...
  std::shared_ptr < colvar_grid_gradient > grad_ptr =
      std::make_shared < colvar_grid_gradient > (gradfile, count_ptr);

  if (!grad_ptr || grad_ptr->nd == 0 || grad_ptr->nd != grad_ptr->multiplicity()
      || proxy->cvmodule->get_error() != COLVARS_OK) {
    // catch constructor failure
    proxy->cvmodule->error("Error reading gradient grid.");
    return proxy->cvmodule->get_error();
  }
