...
cleanup_files() {
  for script in test*.namd testres*.namd ; do
    ...
    for f in ${script%.namd}.*{state,state.stripped,out,traj,coor,vel,xsc,dcd,pmf,hills,grad,force,count,histogram?.dat,corrfunc.dat,histogram?.dx,count.dx,pmf.dx}
    do
