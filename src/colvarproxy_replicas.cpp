...
void colvarproxy::set_replicas_mpi_communicator(replicas_mpi_comm_t comm)
{
  ...
  if (comm != MPI_COMM_NULL) {
    ...
    cvm::log_static(cvmodule, "Enabling multiple replicas: this is replica number " +
...
int colvarproxy::check_replicas_enabled()
{
#ifdef COLVARS_MPI
...
int colvarproxy::replica_index()
{
  return replicas_mpi_rank;
}


int colvarproxy::num_replicas()
{
  return replicas_mpi_num;
}


void colvarproxy::replica_comm_barrier()
{
#ifdef COLVARS_MPI
...
int colvarproxy::replica_comm_recv(char *buffer, int buffer_length, int source_rank)
{
#ifdef COLVARS_MPI
  MPI_Status status;
