...
template<class T>
class colvar_grid : public colvar_grid_params, public colvarparse {
    //protected:
...
public:
    ...
    int init_from_colvars(std::vector<colvar *> const &colvars,
        ...
        if (!config.empty()) {
            ...
            if (params) {
                cvm::error("Error: init_from_colvars was passed both a grid config and a template grid.",
                           COLVARS_BUG_ERROR);
...
/// n-dim colvar space
/// TODO: How to change it properly, this is dirty, in particular maybe create a colvar_grid_coefficients
/// multiply everything by a hundred ?
    ...
    inline int local_sample_count(int radius) {
        ...
        switch (nd) {
            ...
            case 3:
                ...
                break;
            default:
                cvm::error("Error: local_sample_count is not implemented for grids of dimension > 3",
...
public:
    ...
    virtual inline cvm::real value_output(std::vector<int> const &ix,
        ...
        if (samples) {
            return ((s = samples->value(ix)) > 0) ? (data[address(ix) + imult] / cvm::real(s)) : 0.0;
        } else {
...
public:
    ...
    inline void acc_force_kernelized(std::vector<cvm::real> const &cv, cvm::real const *forces, bool b_smoothed,
                                     std::vector<int> kernelSize) {
        //TODO: finish this, do we need to reimplement acc_value, i don't think so...
        //TODO: potentially use kernelSize as a real vector and convert it as a grid integer size
        ...
        if (b_smoothed) {
            ...
            std::vector<int> ix(num_variables(), 0);
            cvm::real sumWeight = 0;
            cvm::real weight = 0;
            cvm::real twoSigmaSquared = 0;
            for (size_t dim = 0; dim < num_variables(); dim++) {
                twoSigmaSquared += std::pow(kernelSize[dim] * widths[dim], 2);
            }
            twoSigmaSquared *= 2;
            switch (num_variables()) {
                case 1:
                    // for cosine-based function, non-zero between -smoothing and +smoothing
                    imin = value_to_bin_scalar(cv[0], 0) - kernelSize[0];
                    imax = value_to_bin_scalar(cv[0], 0) + kernelSize[0];
                    //TODO: ask if it's ok to loop twice here, maybe take a vector instead
                    for (i = imin; i <= imax; i++) {
                        // Delta is the distance from the center of bin i to the current cv value
                        // Smoothing is expressed in units of width
                        weight = exp(
                            std::pow((cv[0] - static_cast<cvm::real>(bin_to_value_scalar(i, 0))), 2) / (
                                2 * std::pow(kernelSize[0], 2)));
                        sumWeight += weight;
                    }
                    for (i = imin; i <= imax; i++) {
                        // Delta is the distance from the center of bin i to the current cv value
                        ...
                        data[this->address(ix)] += weight / sumWeight;
                    }
                    break;
                case 2:
                    // for cosine-based function, non-zero between -smoothing and +smoothing
                    imin = value_to_bin_scalar(cv[0], 0) - kernelSize[0];
                    imax = value_to_bin_scalar(cv[0], 0) + kernelSize[0];
                    jmin = value_to_bin_scalar(cv[1], 1) - kernelSize[1];
                    jmax = value_to_bin_scalar(cv[1], 1) + kernelSize[1];
                    for (i = imin; i <= imax; i++) {
                        for (j = jmin; j <= jmax; j++) {
                            // Delta is the distance from the center of bin i,j to the current cv value
                            // Smoothing is expressed in units of width
                            cvm::real delta0Squared = (std::pow(
                                (cv[0] - static_cast<cvm::real>(bin_to_value_scalar(i, 0))), 2)) *
                                    widths[0] * widths[0];
                            cvm::real delta1Squared = (std::pow(
                                (cv[1] - static_cast<cvm::real>(bin_to_value_scalar(i, 1))), 2)) * widths[1] * widths[1];
                            // weight function has integral dx, same as the discrete case (function equal to 1 over one bin)
                            weight = exp(delta0Squared + delta1Squared / twoSigmaSquared);
                            sumWeight += weight;
                        }
                    }
                    for (i = imin; i <= imax; i++) {
                ...
                case 3:
                    ...
                    jmin = value_to_bin_scalar(cv[1], 1) - kernelSize[1];
                    jmax = value_to_bin_scalar(cv[1], 1) + kernelSize[1];
                    kmin = value_to_bin_scalar(cv[2], 2) - kernelSize[2];
                    kmax = value_to_bin_scalar(cv[2], 2) + kernelSize[2];
                    for (i = imin; i <= imax; i++) {
                        for (j = jmin; j <= jmax; j++) {
                            for (k = kmin; k <= kmax; k++) {
                                // Delta is the distance from the center of bin i,j to the current cv value
                                ...
                                cvm::real delta1Squared = (std::pow(
                                    (cv[1] - static_cast<cvm::real>(bin_to_value_scalar(i, 1))), 2)) * widths[1] * widths[1];
                                cvm::real delta2Squared = (std::pow(
                                    (cv[1] - static_cast<cvm::real>(bin_to_value_scalar(i, 2))), 2)) * widths[2] * widths[2];;
                                // weight function has integral dx, same as the discrete case (function equal to 1 over one bin)
                                weight = exp(delta0Squared + delta1Squared + delta2Squared / twoSigmaSquared);
                                sumWeight += weight;
                            }
                        }
                    }
                        ...
                        for (j = jmin; j <= jmax; j++) {
                            for (k = kmin; k <= kmax; k++) {
                                ...
                                cvm::real delta0Squared = (std::pow(
                                    (cv[0] - static_cast<cvm::real>(bin_to_value_scalar(i, 0))), 2));
                                cvm::real delta1Squared = (std::pow(
                                    (cv[1] - static_cast<cvm::real>(bin_to_value_scalar(i, 1))), 2));
                                cvm::real delta2Squared = (std::pow(
                                    (cv[1] - static_cast<cvm::real>(bin_to_value_scalar(i, 2))), 2));
                                // weight function has integral dx, same as the discrete case (function equal to 1 over one bin)
                                weight = exp(delta0Squared + delta1Squared + delta2Squared / twoSigmaSquared);
                                ix[0] = i;
                                ix[1] = j;
            ...
            acc_force(bin, forces);
        }
    }

    /// Increment the counter at given position
    inline void incr_count_kernel(std::vector<int> const &ix, cvm::real const &weight) {
        data[this->address(ix)] += weight;
    }
