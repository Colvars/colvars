...
int colvar::map_total::init(std::string const &conf)
{
  ...
  if ((volmap_name.size() > 0) && (volmap_id >= 0)) {
    error_code |=
        cvm::error_static(cvmodule, "Error: mapName and mapID are mutually exclusive.\n", COLVARS_INPUT_ERROR);
  }
  ...
  if (get_keyval(conf, "atomWeights", atom_weights, atom_weights)) {
    if (!atoms) {
      error_code |= cvm::error_static(cvmodule, "Error: weights can only be assigned when atoms "
                               "are selected explicitly in Colvars.\n",
                               COLVARS_INPUT_ERROR);
    } else {
      if (atoms->size() != atom_weights.size()) {
        error_code |= cvm::error_static(cvmodule, "Error: if defined, the number of weights ("+
