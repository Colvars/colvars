...
{
  return cvmodule->error("Error: selecting volumetric maps is not available.\n",
...
{
  return cvmodule->error("Error: selecting volumetric maps by name is not "
                    "available.\n", COLVARS_NOT_IMPLEMENTED);
}
...
void colvarproxy_volmaps::clear_volmap(int index)
{
  if (((size_t) index) >= volmaps_ids.size()) {
    cvmodule->error("Error: trying to unrequest a volumetric map that was not "
