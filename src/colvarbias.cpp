...
int colvarbias::init_dependencies() {
  ...
  if (features().size() == 0) {
    ...
    init_feature(f_cvb_opt_cv_params, "variables parameters optimization",
                 f_type_dynamic);
    require_feature_children(f_cvb_opt_cv_params, f_cv_opt_params);
  ...
  // only rad can optimize colvar internal parameters so far
