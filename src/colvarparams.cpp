...
colvarparams::colvarparams(colvarmodule* cvmodule_in): cvmodule(cvmodule_in)
{}
...
void const *colvarparams::get_param_ptr(std::string const &param_name)
{
  ...
  cvm::error_static(cvmodule, "Error: parameter \""+param_name+"\" not found.\n", COLVARS_INPUT_ERROR);
  return NULL;
}
...
{
  if (param_map.count(param_name) > 0) {
    return cvm::error_static(cvmodule, "Error: parameter \""+param_name+"\" cannot be "
